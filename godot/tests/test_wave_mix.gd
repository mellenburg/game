...
## start_asteroid_wave now mixes 20 bodies across small / medium /
...
func test_wave_total_count_is_20() -> void:
	...
	sd.start_asteroid_wave()
	var wave: AsteroidWave = sd.asteroid_waves[0]
...
func test_size_class_counts_within_bands_across_seeds() -> void:
	...
	# counts must sum to ASTEROID_WAVE_COUNT.
...
func test_decaying_count_in_band_and_only_heavy() -> void:
	...
	# medium or large body (small bodies are always plain asteroids).
	for s in range(TRIAL_COUNT):
...
func test_wave_specs_carry_density_and_composition() -> void:
	...
	for entry: Dictionary in wave.pending:
		...
		assert_true(
			comp >= 0 and comp < AsteroidPhysics.COMP_TABLE.size(),
...
func test_extra_large_mass_band_is_disjoint_from_large() -> void:
	...
	assert_close(
		AsteroidPhysics.LARGE_MASS_MAX_KG,
		AsteroidPhysics.EXTRA_LARGE_MASS_MIN_KG,
		1.0e-3,
		"large band ceiling and xl floor must meet exactly",
	)
	assert_true(
		AsteroidPhysics.EXTRA_LARGE_MASS_MAX_KG
		> AsteroidPhysics.EXTRA_LARGE_MASS_MIN_KG,
...
# small impactors are always plain asteroids. Coverage piggybacks on
