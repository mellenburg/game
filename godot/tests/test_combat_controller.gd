...
# MassCenterOrbit.MASS_CENTER_RADIUS_KM is now a runtime-mutable static var (each
# mission may set its own body's radius), so a const initialiser can't
# read it. The tests run on the MassCenter defaults; pin the literal here.
...
func test_exclude_reserved_drops_only_reserved_target_ids() -> void:
	...
	var parts := _make_controller()
	var cc: CombatController = parts[0]
	var a := _make_attacker(MASS_CENTER_RADIUS_KM + 5000.0, 1)
	var b := _make_attacker(MASS_CENTER_RADIUS_KM + 5000.0, 1)
...
func test_railgun_fire_reserves_target_then_releases_on_slug_arrival() -> void:
	...
	var attacker := _make_attacker(MASS_CENTER_RADIUS_KM + 5000.0, 0)
	# Place enemy 3000 km laterally — well outside MassCenter's 6371 km
	# disc relative to the attacker, so LOS is clear and the recoil
	# stays within the safety envelope.
	var enemy := _make_enemy(
		Vector3(MASS_CENTER_RADIUS_KM + 5000.0, 3000.0, 0.0), 1
	)
...
func test_second_railgun_skips_reserved_target() -> void:
	...
	var parts := _make_controller()
	var cc: CombatController = parts[0]
	var attacker_a := _make_attacker(MASS_CENTER_RADIUS_KM + 5000.0, 0)
	var attacker_b := _make_attacker(MASS_CENTER_RADIUS_KM + 5000.0, 0)
	var enemy := _make_enemy(
		Vector3(MASS_CENTER_RADIUS_KM + 5000.0, 3000.0, 0.0), 1
	)
