...
## AsteroidPhysics tests. Pure RefCounted module: burn-up threshold,
...
func test_burn_up_threshold() -> void:
	...
	assert_true(AsteroidPhysics.is_burn_up(0.0))
	assert_true(AsteroidPhysics.is_burn_up(1.0))
	assert_true(AsteroidPhysics.is_burn_up(AsteroidPhysics.BURN_UP_THRESHOLD_KG - 1.0))
	# At and above the threshold the body counts as making it through.
	assert_false(AsteroidPhysics.is_burn_up(AsteroidPhysics.BURN_UP_THRESHOLD_KG))
	assert_false(AsteroidPhysics.is_burn_up(1.0e9))
...
func test_hp_scales_linearly_with_mass_and_density() -> void:
	...
	var hp_a := AsteroidPhysics.hp_for(1.0e6, 3.0)
	var hp_double_mass := AsteroidPhysics.hp_for(2.0e6, 3.0)
	var hp_double_density := AsteroidPhysics.hp_for(1.0e6, 6.0)
	assert_close(hp_double_mass, hp_a * 2.0, 1.0e-3)
	assert_close(hp_double_density, hp_a * 2.0, 1.0e-3)
	assert_close(AsteroidPhysics.hp_for(0.0, 3.0), 0.0, 1.0e-9)
	assert_close(AsteroidPhysics.hp_for(1.0e6, 0.0), 0.0, 1.0e-9)
...
func test_damage_radii_cube_root_scaling() -> void:
	...
	var r_small := AsteroidPhysics.damage_radii_km(1.0e6)
	var r_big := AsteroidPhysics.damage_radii_km(1.0e9)
...
func test_damage_tier_progression() -> void:
	...
	assert_eq(
		AsteroidPhysics.damage_tier_for_mass(1.0e3),
		AsteroidPhysics.TIER_NONE,
	)
	assert_eq(
		AsteroidPhysics.damage_tier_for_mass(AsteroidPhysics.BURN_UP_THRESHOLD_KG),
		AsteroidPhysics.TIER_LIGHT,
	)
	assert_eq(
		AsteroidPhysics.damage_tier_for_mass(1.0e7),
		AsteroidPhysics.TIER_MODERATE,
	)
	assert_eq(
		AsteroidPhysics.damage_tier_for_mass(1.0e9),
		AsteroidPhysics.TIER_HEAVY,
	)
...
func test_composition_sample_in_range() -> void:
	...
	rng.seed = 42
	for _i in range(200):
		var s := AsteroidPhysics.sample_density(rng)
		var comp: int = int(s["composition"])
		var d: float = float(s["density"])
		assert_true(comp >= 0 and comp < AsteroidPhysics.COMP_TABLE.size())
		var lo: float = float(AsteroidPhysics.COMP_TABLE[comp][1])
		var hi: float = float(AsteroidPhysics.COMP_TABLE[comp][2])
		assert_true(
...
func test_composition_weights_sum_to_one() -> void:
	...
	var total := 0.0
	for row in AsteroidPhysics.COMP_TABLE:
		total += float(row[0])
	assert_close(total, 1.0, 1.0e-6)


func test_composition_name_known() -> void:
	assert_eq(AsteroidPhysics.composition_name(AsteroidPhysics.COMP_S_TYPE), "S-type")
	assert_eq(AsteroidPhysics.composition_name(AsteroidPhysics.COMP_M_TYPE), "M-type")
	assert_eq(AsteroidPhysics.composition_name(-1), "unknown")
	assert_eq(AsteroidPhysics.composition_name(99), "unknown")
...
func test_damage_areas_are_exclusive_rings() -> void:
	...
	var radii: Dictionary = AsteroidPhysics.damage_radii_km(1.0e9)
	var areas: Dictionary = AsteroidPhysics.damage_areas_km2(1.0e9)
...
func test_sample_log_uniform_covers_decades() -> void:
	...
	var n := 2000
	for _i in range(n):
		var v := AsteroidPhysics.sample_log_uniform(rng, lo, hi)
...
func test_mass_log_norm_endpoints() -> void:
	...
	assert_close(AsteroidPhysics.mass_log_norm(0.0), 0.0, 1.0e-9)
	assert_close(AsteroidPhysics.mass_log_norm(AsteroidPhysics.BURN_UP_THRESHOLD_KG), 0.0, 1.0e-9)
	var top: float = AsteroidPhysics.MARKER_LOG_REF_KG * pow(10.0, AsteroidPhysics.MARKER_LOG_DECADES)
	assert_close(AsteroidPhysics.mass_log_norm(top), 1.0, 1.0e-9)
	# Off the top anchor the norm clamps at 1.0 — no overflow into >1.
	assert_close(AsteroidPhysics.mass_log_norm(top * 100.0), 1.0, 1.0e-9)
	# Half-way (in log space) lands at 0.5 within float tolerance.
	var mid: float = AsteroidPhysics.MARKER_LOG_REF_KG * pow(10.0, AsteroidPhysics.MARKER_LOG_DECADES * 0.5)
	assert_close(AsteroidPhysics.mass_log_norm(mid), 0.5, 1.0e-6)
...
func test_mass_for_hp_inverts_hp_for() -> void:
	...
	for m in [1.0e4, 1.0e7, 1.0e10]:
		for d in [1.6, 3.4, 8.0]:
			var hp := AsteroidPhysics.hp_for(m, d)
			var roundtrip := AsteroidPhysics.mass_for_hp(hp, d)
			assert_close(roundtrip, m, m * 1.0e-6)
	# Edge cases: zero / negative density returns zero (no divide).
	assert_close(AsteroidPhysics.mass_for_hp(100.0, 0.0), 0.0, 1.0e-9)
	assert_close(AsteroidPhysics.mass_for_hp(100.0, -1.0), 0.0, 1.0e-9)
	# Negative HP clamps at zero so a damage-overshoot bug can't drive
	# mass negative.
	assert_close(AsteroidPhysics.mass_for_hp(-50.0, 3.4), 0.0, 1.0e-9)
