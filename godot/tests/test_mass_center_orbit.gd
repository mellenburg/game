...
func test_compute_periapsis_circular_equals_radius() -> void:
	var radius := MassCenterOrbit.BODY_RADIUS_KM + 800.0
...
func test_clamp_dv_noop_when_already_safe() -> void:
	...
	var safe := MassCenterOrbit.clamp_dv_for_min_periapsis(
		o.r, o.v, dv, MassCenterOrbit.BODY_RADIUS_KM + 1.0
	)
...
func test_clamp_dv_blocks_deorbit_burn() -> void:
	...
	var threshold := MassCenterOrbit.BODY_RADIUS_KM + 1.0
...
func test_clamp_dv_zero_when_already_unsafe() -> void:
	...
	var pos := Vector3(MassCenterOrbit.BODY_RADIUS_KM + 200.0, 0.0, 0.0)
	var vel := Vector3(0.0, 6.0, 0.0)  # too slow for 200 km circular
	assert_true(MassCenterOrbit.compute_periapsis(pos, vel) < MassCenterOrbit.BODY_RADIUS_KM)
	var dv := Vector3(0.0, -0.5, 0.0)
	var safe := MassCenterOrbit.clamp_dv_for_min_periapsis(
		pos, vel, dv, MassCenterOrbit.BODY_RADIUS_KM + 1.0
	)
...
func test_make_circular_altitude_and_eccentricity() -> void:
	...
	assert_close(o.norm_r, MassCenterOrbit.BODY_RADIUS_KM + alt, 1.0e-3)
	assert_close(o.r_p, MassCenterOrbit.BODY_RADIUS_KM + alt, 1.0e-2)
...
func test_make_circular_altitude_invariant_under_propagation() -> void:
	...
	var alt := 500.0
	var radius := MassCenterOrbit.BODY_RADIUS_KM + alt
...
func test_time_to_impact_circular_orbit_never_hits() -> void:
	...
	# circuit on r_p > BODY_RADIUS_KM should return INF without
...
func test_time_to_impact_underground_returns_zero() -> void:
	...
	var o := MassCenterOrbit.new(
		Vector3(MassCenterOrbit.BODY_RADIUS_KM * 0.5, 0.0, 0.0),
		Vector3(0.0, 5.0, 0.0),
	)
