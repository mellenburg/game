...
func test_reset_seeds_starting_tier_only_for_each_chain() -> void:
	...
	assert_true(r.is_unlocked("launch_capacity_15"))
	assert_true(r.is_unlocked("ground_defense_1"))
	assert_false(r.is_unlocked("laser_advanced"))
	assert_false(r.is_unlocked("launch_capacity_18"))
...
func test_starting_capacities_are_first_tier_values() -> void:
	var r := _make_research()
	assert_eq(r.launch_capacity(), 15)
...
func test_can_unlock_refuses_when_prereq_missing() -> void:
	...
	assert_false(r.can_unlock("launch_capacity_21"))
...
func test_chain_unlock_promotes_capacity() -> void:
	var r := _make_research()
	assert_true(r.unlock("launch_capacity_18"))
	assert_eq(r.launch_capacity(), 18)
	assert_true(r.unlock("launch_capacity_21"))
	assert_eq(r.launch_capacity(), 21)
	assert_true(r.unlock("launch_capacity_24"))
	assert_eq(r.launch_capacity(), 24)
