...
func _round_trip(lat_deg: float, lon_deg: float, phase: float) -> Vector2:
	var radius := MassCenterOrbit.BODY_RADIUS_KM
...
func test_round_trip_with_rotation_phase() -> void:
...
func test_rotation_phase_rotates_position() -> void:
	# At lat=0, lon=0 the surface point sits in the equatorial plane;
	# advancing rotation_phase should sweep it through that plane such
