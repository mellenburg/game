...
func _make_decaying() -> MassCenterOrbit:
	...
	var r_a := MassCenterOrbit.BODY_RADIUS_KM + APOGEE_ALT_KM
	var a := 0.5 * (r_p + r_a)
...
func test_apogee_matches_spec() -> void:
	...
	assert_close(o.r_a, MassCenterOrbit.BODY_RADIUS_KM + APOGEE_ALT_KM, 1.0e-2)


func test_perigee_matches_spec() -> void:
	var o := _make_decaying()
	assert_close(o.r_p, MassCenterOrbit.BODY_RADIUS_KM + PERIGEE_ALT_KM, 1.0e-2)
...
func test_spawn_is_near_apogee() -> void:
	...
	var o := _make_decaying()
	assert_true(
		o.norm_r > MassCenterOrbit.BODY_RADIUS_KM + 0.5 * APOGEE_ALT_KM,
...
func test_spiral_final_segment_meets_impact_altitude() -> void:
	...
	# radius (EARTH_RADIUS + 60 km for Earth defaults) — that is where the
