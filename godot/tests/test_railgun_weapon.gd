...
# MassCenterOrbit.BODY_RADIUS_KM is a runtime-mutable static var (per-
...
func _make_player(radius: float = BODY_RADIUS_KM + 5000.0) -> FakeSat:
	var s := FakeSat.new()
	s.team = 0
...
func test_fire_applies_damage_and_drains_energy_and_locks_cooldown() -> void:
	...
	var target := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 3000.0, 0.0))
...
func test_does_not_engage_same_team() -> void:
	...
	ally.team = 0
	var radius: float = BODY_RADIUS_KM + 500.0
...
func test_does_not_engage_when_los_blocked() -> void:
	...
	# should refuse fire.
	var w := RailgunWeapon.new()
	var attacker := _make_player(BODY_RADIUS_KM + 500.0)
...
func test_does_not_engage_inert_asteroid() -> void:
	...
	var w := RailgunWeapon.new()
	var attacker := _make_player()
	var enemy := _make_enemy(Vector3(BODY_RADIUS_KM + 5000.0, 5000.0, 0.0))
	enemy.inert = true
	assert_false(w.is_target_in_engagement_envelope(attacker, enemy))
	assert_false(w.fire(attacker, enemy, 1.0))
...
func test_refuses_shot_that_exceeds_max_orbital_radius() -> void:
	...
	attacker.max_orbital_radius_km = BODY_RADIUS_KM + 510.0
	var enemy := FakeSat.new()
	enemy.team = 1
	var ey := BODY_RADIUS_KM + 500.0
...
func test_pick_target_does_not_gate_on_enabled_flag() -> void:
	...
	var attacker := _make_player()
	attacker.railgun_enabled = false
	var enemy := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 3000.0, 0.0))
