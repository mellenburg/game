...
func _make_player(pos: Vector3 = Vector3(BODY_RADIUS_KM + 500.0, 0.0, 0.0)) -> FakeSat:
	var s := FakeSat.new()
	s.team = 0
	s.orbit.r = pos
	return s
...
func test_demands_cooling_after_fire() -> void:
	...
	var target := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 1000.0, 0.0))
...
func test_does_not_engage_same_team() -> void:
	...
	var attacker := _make_player()
	attacker.energy = STARTING_ENERGY_J
	var ally := _make_player(Vector3(BODY_RADIUS_KM + 500.0, 1000.0, 0.0))
...
func test_does_not_engage_when_los_blocked() -> void:
	var w := LaserWeapon.new()
	var attacker := _make_player(Vector3(BODY_RADIUS_KM + 500.0, 0.0, 0.0))
	attacker.energy = STARTING_ENERGY_J
	var enemy := _make_enemy(Vector3(-(BODY_RADIUS_KM + 500.0), 0.0, 0.0))
...
func test_does_not_engage_inert_asteroid() -> void:
	...
	var attacker := _make_player()
	attacker.energy = STARTING_ENERGY_J
	var enemy := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 1000.0, 0.0))
	enemy.inert = true
	assert_false(w.is_target_in_engagement_envelope(attacker, enemy))
...
func test_damage_scales_with_distance() -> void:
	...
	var atk_near := _make_player(Vector3(BODY_RADIUS_KM + 500.0, 0.0, 0.0))
	var atk_far := _make_player(Vector3(BODY_RADIUS_KM + 500.0, 0.0, 0.0))
	...
	var tgt_near := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 2000.0, 0.0))
	var tgt_far := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 8000.0, 0.0))
...
func test_does_not_engage_beyond_max_range() -> void:
	...
	var enemy := _make_enemy(
		Vector3(BODY_RADIUS_KM + 500.0, far, 10000.0)
	)
	assert_false(w.is_target_in_engagement_envelope(attacker, enemy))
	assert_false(w.fire(attacker, enemy, 1.0))
	assert_close(enemy.hp, 100.0)
...
func test_engagement_range_gates_fire_only_with_fire_control_on() -> void:
	...
	var enemy := _make_enemy(Vector3(BODY_RADIUS_KM + 500.0, 6000.0, 0.0))
